...
describe('the weight slider', () => {
  const slider = (c: HTMLElement) => c.querySelector('.ffw-slider__input') as HTMLInputElement | null;

  /*
   * This replaced a hold-then-drag on the cell that did not work: `touch-action`
   * is read when the touch begins, so a cell inside a scrolling list is already
   * the scroller's, and arming later gets a `pointercancel` on the first move.
   * A native range has no such problem, and gets keyboard and screen-reader
   * behaviour without reimplementing either.
   */
  it('appears in the weight editor', () => {
    const row = setup({ openField: 'weight' });
    expect(slider(row.container)).not.toBeNull();
  });

  it('is not in the reps editor, where one tap on a chip already wins', () => {
    const row = setup({ openField: 'effort' });
    expect(slider(row.container)).toBeNull();
  });

  it('moves in plate increments, never continuously', () => {
    const row = setup({ openField: 'weight' });
    expect(slider(row.container)?.step).toBe('2.5');
  });

  it('scales its range to what you actually lift', () => {
    // A fixed 0–200 leaves someone curling 12 kg with no usable resolution.
    const heavy = setup({ openField: 'weight', set: { weightKg: 100 } });
    expect(Number(slider(heavy.container)?.max)).toBe(200);

    const light = setup({ openField: 'weight', set: { weightKg: 12.5 } });
    expect(Number(slider(light.container)?.max)).toBe(25);
  });

  it('falls back to last session when this set is empty', () => {
    // GHOST.weightKg is 100, so the range is built around that rather than zero.
    const row = setup({ openField: 'weight', set: { weightKg: null } });
    expect(Number(slider(row.container)?.max)).toBe(200);
  });

  it('still has somewhere to travel with no value and no history', () => {
    // Falls back to a 20 kg anchor, so an empty bar gets a 0–40 range rather
    // than a slider pinned at zero with nowhere to go.
    const row = setup({ openField: 'weight', set: { weightKg: null }, ghost: undefined });
    expect(Number(slider(row.container)?.max)).toBe(40);
  });

  it('commits as it moves, so the value is never trapped in the control', () => {
    const row = setup({ openField: 'weight' });
    const input = slider(row.container) as HTMLInputElement;
    fireEvent.change(input, { target: { value: '60' } });
    expect(row.onEdit).toHaveBeenCalledWith({ weightKg: 60 });
  });

  it('keeps the number field alongside, for anything unusual', () => {
    const row = setup({ openField: 'weight' });
    expect(row.container.querySelector('.ff-number')).not.toBeNull();
  });
});
